...
constexpr auto PyTupleUnpack(tuple_t tuple, std::index_sequence<idx...>) {
  return PyTuple_Pack(
      std::tuple_size<tuple_t>::value, ToObject(std::get<idx>(tuple)).get()...
  );
}
...
struct TupleSlice {
  using type =
      typename TupleSliceImpl<tuple_t, typename MakeSlice<low, high>::type>::type;
};
}  // namespace detail
...
class Tuple : public Object {
  static_assert(
      std::conjunction_v<isPythonBaseType<Ts>...>,
 ...
 public:
  ...
  Tuple(const std::tuple<Ts...>& tuple) {
    m_pObject =
        detail::PyTupleUnpack(tuple, std::make_index_sequence<sizeof...(Ts)>());
    if (!m_pObject) {
      throw Error();
    }
  }

  Tuple(PyObject* pTupleObj) : Object(pTupleObj) {
  ...
  Tuple(const Tuple& other) : Object(other) {}
  ...
  Tuple(Tuple&& other) noexcept : Object(std::move(other)) {}
  ...
  Tuple(const Object& other) : Object(other) {
  ...
  // PyTuple_Check they cannot be defined noexcept!

  [[nodiscard]] constexpr size_t size() const noexcept { return sizeof...(Ts); }
  ...
  typename std::tuple_element_t<idx, std::tuple<Ts...>> at() const {
    ...
    return python_cast<typename std::tuple_element_t<idx, std::tuple<Ts...>>>(
        pItem
    );
  }
