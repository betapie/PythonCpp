...
template <typename U>
struct isPythonBaseType<List<U>> : std::conditional_t<
                                       isPythonBaseType<U>::value,
                                       std::true_type,
                                       std::false_type> {};
...
// note: no implementations for PyLong_FromDouble... and other implicit
// conversions. If you want your Object to be of a certain type, be explicit
// about the type you pass in
...
[[nodiscard]] Object ToObject(const bool& val) {
  Object pObject =
      PyBool_FromLong(val ? static_cast<long>(1) : static_cast<long>(0));
  if (!pObject) {
    throw Error();
  }
  return pObject;
...
// python_cast converts the contents of a Object to your desired type, if
// possible. for example a conversion to long will succeed, if the Python object
// pointed to is of type int or has
// __int__() implemented (can be converted to int). This cast will also check
// for possible overflow note: Python C API allows for conversion from int to
// double etc. These will not be supported. Please cast them accordingly and if
// you need conversions cast them manually

template <
    typename T,
    std::enable_if_t<!std::is_base_of_v<Object, T>, int> =
        0>  // base template, this will not do anything except
            // warning about wrong types
...
template <>
[[nodiscard]] unsigned long long python_cast<unsigned long long>(
    const Object& pyObj
) {
...
template <>
[[nodiscard]] std::complex<double> python_cast<std::complex<double>>(
    const Object& pyObj
) {
