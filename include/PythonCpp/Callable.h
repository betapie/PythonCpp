...
template <typename CharT>
typename BuildType<std::basic_string<CharT>>::type PrepareForBuild(
 ...
 *  meaning that callable(object) in Python evaluates to True. A Callable can
 * not be constructed from a Object or PyObject* which does not satisfy that
 * requirement. A Error will be thrown in that instance.
 */
...
class PYCPP_API Callable : public Object {
 public:
  ...
  // Take ownership of an existing PyObject which points to a callable Python
  // Object Will throw Error if object pointed to by PyObject* is not callable
  ...
  // Note: No move construction/assignment from Object, because due to the
  // PyCallable_Check they cannot be defined noexcept!
...
// Shortcut function for calling a function or method which is owned by
// owningObject with args functionArgs
template <typename... Args>
Object CallFunction(
    const Object& owningObject,
