...
class List : public Object {
 ...
 private:
  // Helper proxy to return some kind of reference type from subscription
  // operator
  class Reference {
   public:
 ...
 public:
  ...
  template <
      typename Container,
      typename val_t = typename Container::value_type,
      std::enable_if_t<isPythonBaseType_v<val_t>, int> = 0>
  ...
  // Take ownership of an existing PyObject which points to a Python List or
  // subtype of List Will throw Error if object pointed to by PyObject* is not
  // of List type
  List(PyObject* pListObj) : Object(pListObj) {
    if (PyList_Check(pListObj) == 0) {
      throw Error("PyObject not of List type");  // TODO more info
    }
  }

  List(const List& other) : Object(other) {}
  ...
  List(List&& other) noexcept : Object(std::move(other)) {}
  ...
  List(const Object& other) : Object(other) {
  ...
  // PyList_Check they cannot be defined noexcept!
